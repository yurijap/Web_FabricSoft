...
export class Sunburst extends Partition {
    constructor() {
        super(...arguments);
        this._tag = "sunburst";
        this._partitionLayout = d3hierarchy.partition();
        /**
         * A list of node slice elements in a [[Sunburst]] chart.
         *
         * @default new ListTemplate<Slice>
         */
        this.slices = this.addDisposer(new ListTemplate(Template.new({}), () => Slice._new(this._root, {
         ...
         */
        this.labels = this.addDisposer(new ListTemplate(Template.new({}), () => RadialLabel._new(this._root, {
    ...
    _prepareChildren() {
        ...
        // These settings are only consumed in `_updateVisuals` (arc bounds, radius
        // and per-slice angle math), which the base only runs on value/size
        // changes — so redraw when they change dynamically.
        if (this.isDirty("startAngle") || this.isDirty("endAngle") || this.isDirty("radius") || this.isDirty("innerRadius")) {
            this._updateVisuals();
        }
...
Sunburst.classNames = Partition.classNames.concat([Sunburst.className]);
