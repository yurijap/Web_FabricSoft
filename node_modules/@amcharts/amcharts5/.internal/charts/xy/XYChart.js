...
export class XYChart extends SerialChart {
    constructor() {
         ...
         */
        this.topAxesContainer = this.chartContainer.children.push(Container.new(this._root, { width: p100, layout: this._root.verticalLayout })._markC("width", "layout"));
        /**
         ...
         * @default Container.new()
         */
        this.yAxesAndPlotContainer = this.chartContainer.children.push(Container.new(this._root, { width: p100, height: p100, layout: this._root.horizontalLayout })._markC("width", "height", "layout"));
        /**
         ...
         * @default Container.new()
         */
        this.bottomAxesContainer = this.chartContainer.children.push(Container.new(this._root, { width: p100, layout: this._root.verticalLayout })._markC("width", "layout"));
        /**
         ...
         * @default Container.new()
         */
        this.leftAxesContainer = this.yAxesAndPlotContainer.children.push(Container.new(this._root, { height: p100, layout: this._root.horizontalLayout })._markC("height", "layout"));
        /**
         ...
         * @default Container.new()
         */
        this.plotsContainer = this.yAxesAndPlotContainer.children.push(Container.new(this._root, { width: p100, height: p100, maskContent: false })._markC("width", "height", "maskContent"));
        /**
         ...
         * @default Container.new()
         */
        this.plotContainer = this.plotsContainer.children.push(Container.new(this._root, { width: p100, height: p100 })._markC("width", "height"));
        /**
         ...
         * @default Container.new()
         */
        this.topPlotContainer = this.plotsContainer.children.push(Container.new(this._root, { width: p100, height: p100 })._markC("width", "height"));
        /**
         ...
         * @default Container.new()
         */
        this.gridContainer = this.plotContainer.children.push(Container.new(this._root, { width: p100, height: p100, isMeasured: false })._markC("width", "height", "isMeasured"));
        /**
         ...
         * @default Container.new()
         */
        this.topGridContainer = Container.new(this._root, { width: p100, height: p100, isMeasured: false })._markC("width", "height", "isMeasured");
        /**
         ...
         * @default Container.new()
         */
        this.rightAxesContainer = this.yAxesAndPlotContainer.children.push(Container.new(this._root, { height: p100, layout: this._root.horizontalLayout })._markC("height", "layout"));
        /**
    ...
    _afterNew() {
        ...
        zoomOutButton.states.lookup("default")._setC("opacity", 1);
        ...
        // is interactive
        plotContainer._setC("interactive", true);
        plotContainer._setC("interactiveChildren", false);
        plotContainer._setC("background", Rectangle.new(root, {
    ...
    _applyThemes(force = false) {
        ...
        // Recolorize all series that might've not been added before theme was
    ...
    _prepareChildren() {
        ...
        if (this.isDirty("scrollbarX")) {
            ...
            if (scrollbarX !== previous) {
                ...
                if (scrollbarX) {
                    this._placeScrollbar("scrollbarX", scrollbarX, this.topAxesContainer, this.bottomAxesContainer);
        ...
        if (this.isDirty("scrollbarY")) {
            ...
            if (scrollbarY !== previous) {
                ...
                if (scrollbarY) {
                    this._placeScrollbar("scrollbarY", scrollbarY, this.rightAxesContainer, this.leftAxesContainer, true);
    ...
    /**
     * Puts a scrollbar into one of two containers, depending on its `opposite`
     * setting, and moves it over if the setting changes later. `unshift` is for
     * the left container, which fills left to right, so the scrollbar has to go
     * in at the front to end up outside the axes.
     *
     * @ignore
     */
    _placeScrollbar(key, scrollbar, container, oppositeContainer, unshift) {
        const place = () => {
            const opposite = scrollbar.get("opposite");
            const target = opposite ? oppositeContainer : container;
            const children = target.children;
            const index = (opposite && unshift) ? 0 : children.length - 1;
            if (scrollbar.parent != target || children.indexOf(scrollbar) != index) {
                // A vertical layout only ever sets a child's private `y` and a
                // horizontal one only `x`, so whatever the new container does not
                // own would keep the value the previous one gave it. Drop both, the
                // same way a container resets its children when its own layout
                // changes.
                scrollbar.removePrivate("x");
                scrollbar.removePrivate("y");
                if (opposite && unshift) {
                    children.moveValue(scrollbar, 0);
                }
                else {
                    children.moveValue(scrollbar);
                }
                // Inserting a child only marks the container it came from, so make
                // the receiving side re-measure and lay out again.
                target.markDirtyBounds();
                // A vertical scrollbar changes the width of the left/right
                // containers, which the top/bottom padding is derived from. Those
                // widths only settle once the layout has run, hence the wait.
        ...
        this._pushPropertyDisposer(key, scrollbar.on("opposite", () => {
            // only move scrollbars the chart placed itself
            const parent = scrollbar.parent;
            if (parent == container || parent == oppositeContainer) {
                place();
            }
        }));
    }
    _processSeries(series) {
        ...
        if (!series.get("baseAxis")) {
            // the chart working it out, not something anyone asked for
            if (yAxis.isType("CategoryAxis") || yAxis.isType("DateAxis")) {
                series._setC("baseAxis", yAxis);
            }
            else {
                series._setC("baseAxis", xAxis);
            }
        }
    ...
    _colorize(series) {
        ...
        // A series coloring its own data items still takes a color of its own: it
        // is what the columns go back to when that is switched off again
        if (colorSet) {
        ...
        // Cycle stroke widths / dash arrays for line series (those that have a
        // `strokes` list), keyed by the series' position — so line series can be
        // told apart by weight and dashiness without relying on color.
        const strokeWidths = this.get("strokeWidths");
        const strokeDasharrays = this.get("strokeDasharrays");
        if (strokeWidths || strokeDasharrays) {
            const strokes = series.strokes;
            if (strokes && strokes.template) {
                const index = this.series.indexOf(series);
                if (strokeWidths && strokeWidths.length > 0) {
                    strokes.template._setC("strokeWidth", strokeWidths[index % strokeWidths.length]);
                }
                if (strokeDasharrays && strokeDasharrays.length > 0) {
                    strokes.template._setC("strokeDasharray", strokeDasharrays[index % strokeDasharrays.length]);
                }
            }
        }
    }
        ...
        if (this.xAxes.indexOf(axis) != -1) {
            if (force || axis.getPrivate("updateScrollbar")) {
                ...
                if (scrollbarX && (!scrollbarX.getPrivate("isBusy") || force)) {
                    scrollbarX._setCRaw("start", start);
                    scrollbarX._setCRaw("end", end);
            ...
            if (force || axis.getPrivate("updateScrollbar")) {
                ...
                if (scrollbarY && (!scrollbarY.getPrivate("isBusy") || force)) {
                    scrollbarY._setCRaw("start", start);
                    scrollbarY._setCRaw("end", end);
