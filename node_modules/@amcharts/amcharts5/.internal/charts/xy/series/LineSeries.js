...
export class LineSeries extends XYSeries {
    ...
    _updateChildren() {
        ...
        if (this.isDirty("stroke")) {
            ...
            this.strokes.template._setC("stroke", stroke);
            const legendStroke = this._legendStroke;
            if (legendStroke) {
                legendStroke.states.lookup("default")._setC("stroke", stroke);
            }
        }
        if (this.isDirty("fill")) {
            const fill = this.get("fill");
            this.fills.template._setC("fill", fill);
            const legendFill = this._legendFill;
            if (legendFill) {
                legendFill.states.lookup("default")._setC("fill", fill);
            }
        }
        if (this.isDirty("fillPattern")) {
            const fillPattern = this.get("fillPattern");
            this.fills.template._setC("fillPattern", fillPattern);
            const legendFill = this._legendFill;
            if (legendFill) {
                legendFill.states.lookup("default")._setC("fillPattern", fillPattern);
            }
        }
            ...
            if (this._axesDirty || this._valuesDirty || this._stackDirty || this.isDirty("vcx") || this.isDirty("vcy") || this._sizeDirty || this.isDirty("connect") || this.isDirty("curveFactory")) {
                ...
                for (let i = startIndex - 1; i >= 0; i--) {
                    ...
                    for (let f = 0, flen = this._valueFields.length; f < flen; f++) {
                        if (!$type.isNumber(dataItem.get(this._valueFields[f]))) {
                            hasValues = false;
                            break;
                        }
                    }
    ...
    createLegendMarker(_dataItem) {
        ...
        if (legendDataItem) {
            ...
            legendStroke.states._createC("disabled", { fill: disabledColor, stroke: disabledColor });
            legendFill.states._createC("disabled", { fill: disabledColor, stroke: disabledColor });
                ...
                if (bulletFunction) {
                    ...
                    if (bullet) {
                        ...
                        if (sprite instanceof Graphics) {
                            sprite.states._createC("disabled", { fill: disabledColor, stroke: disabledColor });
                        }
                        if (sprite) {
                            // legend bullets are created straight from the bullet
                            // function, bypassing `_makeBulletReal`, so apply the same
                            // series-paint inheritance here — otherwise a bullet with
                            // no fill of its own is invisible in the legend
                            this._applyBulletDefaultPaint(sprite);
                            sprite.setAll({
