...
export class RadarLineSeries extends LineSeries {
    _afterNew() {
        super._afterNew();
        this._setC("maskContent", false);
        this.bulletsContainer._setC("maskContent", false);
        this.bulletsContainer._setC("mask", Graphics.new(this._root, {}));
    }
    _handleMaskBullets() {
    }
    ...
    _prepareChildren() {
        ...
        // `connectEnds` is read in `_endLine` during the line rebuild in
        // `_updateChildren` (gated by `_valuesDirty` among others), so force that
...
RadarLineSeries.classNames = LineSeries.classNames.concat([RadarLineSeries.className]);
