...
export class FunnelSeries extends PercentSeries {
    ...
    _afterNew() {
        ...
        if (this.get("orientation") == "vertical") {
            this._setC("layout", this._root.horizontalLayout);
        }
        else {
    ...
    _updateChildren() {
        ...
        // `bottomRatio`/`startLocation`/`endLocation` are read while laying out the
        // slices below (and in `decorateSlice`), so re-run this pass when they change.
        if (this._total > 0 && (this._valuesDirty || this._sizeDirty || this.isDirty("bottomRatio") || this.isDirty("startLocation") || this.isDirty("endLocation"))) {
            const slicesContainer = this.slicesContainer;
            let h;
            if (this.get("orientation") == "vertical") {
    ...
    _fixLayout() {
        ...
        if (this.get("alignLabels")) {
            labelsContainer._setC("position", "relative");
            labelsContainer._setCAll({ isMeasured: true });
            if (orientation == "vertical") {
                this._setC("layout", this._root.horizontalLayout);
                labelsTemplate._setCAll({ centerX: p100, x: p100 });
            }
            else {
                this._setC("layout", this._root.verticalLayout);
                labelsTemplate._setCAll({ centerX: 0, x: 0 });
            }
        }
        else {
            labelsContainer._setCAll({ isMeasured: false, position: "absolute" });
            if (orientation == "vertical") {
                labelsContainer._setCAll({ x: p50 });
                labelsTemplate._setCAll({ centerX: p50, x: 0 });
            }
            else {
                labelsContainer._setCAll({ y: p50 });
                labelsTemplate._setCAll({ centerX: p50, y: 0 });
            }
        }
        this.markDirtySize();
    }
    ...
    _positionBullet(bullet) {
        ...
        if (sprite) {
            ...
            const slice = dataItem.get("slice");
            if (slice) {
                // the slice knows where its own sloping edges are at any location,
                // the bounding box would put the bullet outside a tapered slice
                const point = slice.getPoint(bullet.get("locationX", 0.5), bullet.get("locationY", 0.5));
                sprite.setAll({ x: slice.x() + point.x, y: slice.y() + point.y });
            }
        }
    }
}
