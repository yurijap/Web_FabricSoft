...
const AgenteTokenLog = require('../models/model.agenteTokenLog');
...
async function generarRespuestaAgente(agente, message, rawHistory = [], type = 'public', sessionId = null) {
  ...
  for (const item of agente.llmChain || []) {
    try {
      ...
      if (result && result.reply) {
        // Guardar logs de tokens de forma asíncrona sin bloquear la respuesta al cliente
        if (result.usage) {
          AgenteTokenLog.create({
            provider: item.provider,
            model: item.model,
            ...
            type,
            sessionId,
          }).catch((err) => console.error('Error guardando Log de Tokens:', err));
        }

        return {
          reply: polishReply(result.reply),
          usedConfig: {
            provider: item.provider,
            model: item.model,
...
exports.probarConfiguracion = async (req, res) => {
  try {
    ...
    const result = await generarRespuestaAgente(agente, message, history, 'test');
...
exports.probarPublico = async (req, res) => {
  try {
    ...
    const config = await AgenteIA.findOne({ status: 'active' })
      ...
      .lean();
    const agente = config || DEFAULT_CONFIG;
    const result = await generarRespuestaAgente(agente, message, history, 'public', sessionId);
...
exports.obtenerEstadisticasTokens = async (req, res) => {
  try {
    const ownerClerkId = req.auth?.userId;
    if (!ownerClerkId) {
      return res.status(401).json({ error: 'No autorizado' });
    ...
    const totalStats = await AgenteTokenLog.aggregate([
      {
        $group: {
          _id: null,
    ...
    const byProvider = await AgenteTokenLog.aggregate([
      {
        $group: {
          _id: '$provider',
    ...
    const byType = await AgenteTokenLog.aggregate([
      {
        $group: {
          _id: '$type',
          totalTokens: { $sum: '$totalTokens' },
    ...
    const dailyUsage = await AgenteTokenLog.aggregate([
      {
        $match: {
    ...
    return res.status(200).json({
      success: true,
      stats: {
        summary: totalStats[0] || { totalPrompt: 0, totalCompletion: 0, totalTokens: 0, count: 0 },
        byProvider,
        byType,
        dailyUsage,
      },
    ...
    console.error('Error obteniendo estadisticas de tokens:', error);
    return res.status(500).json({ error: 'No se pudieron obtener las estadisticas de tokens.' });
  }
};
